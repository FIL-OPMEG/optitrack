function [ MovementDataOut, OPMdataOut ] = syncRigidBodyOPM(cfg,MovementData, OPMdata, varargin )
...
if isstruct(MovementData)
    MovementData = struct2table(MovementData);
end

% Trim data
if ~cfg.LengthsAlreadyMatch
    % Get Trigger channel
    if ~isfield(cfg, 'Trigger')
        switch OPMdataType
            case 'spm'
                [~,~,optiChanind] = intersect(cfg.TriggerChannelName, OPMdata.chanlabels);
                cfg.Trigger = OPMdata(optiChanind, :, :);
            case 'ft'
                    pos_of_trig     = ismember(OPMdata.label,cfg.TriggerChannelName);
                    cfg.Trigger  = OPMdata.trial{1}(pos_of_trig,:);
            case 'matrix'
...
%     % Find edges of trigger (where it steps up and steps down)
     [~, samples] = findpeaks(abs(diff(cfg.Trigger)), 'MinPeakHeight', 0.5*max(abs(diff(cfg.Trigger)))); 
